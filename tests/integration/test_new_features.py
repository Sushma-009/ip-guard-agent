...
def test_standard_counsel_rejection_flow():
    """Verify that a standard non-flagged submission goes to PAUSED_FOR_REVIEW and can be successfully rejected by Counsel."""
    client = TestClient(app)
    auth = get_auth_header("user_a_sub", "org_a", "submitter")
    ...
    # Counsel reviews (REJECTS) the submission
    counsel_auth = get_auth_header("user_a_counsel", "org_a", "counsel")
    review_payload = {
        "decision": "REJECT",
        "comment": "Not novel enough, similar prior art exists."
    }
    
    review_res = client.post(f"/submissions/{sub_id}/review", json=review_payload, headers=counsel_auth)
    assert review_res.status_code == 200
    
    # Check that database status has successfully updated to REJECTED and reason has comment
    final_record = get_submission("org_a", sub_id)
    assert final_record["status"] == "REJECTED"
    assert "Rejected by IP Counsel" in final_record["reason"]
    assert "Not novel enough, similar prior art exists" in final_record["reason"]
