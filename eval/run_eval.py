...
# Set test key environment variable if not already set, before importing app
...
from expense_agent.db import initialize_db, create_organization, create_user
from expense_agent.auth import hash_password
from app.fast_api_app import app
from google.adk.cli.fast_api import create_session_service_from_options
...
async def run_single_eval_case(case: Dict[str, Any], client: TestClient, token: str, session_service: Any) -> Dict[str, Any]:
    """Runs a single test case through the authenticated FastAPI /submissions endpoint."""
    ...
    assert response.status_code == 200, f"API submission failed for {case['case_id']}: {response.text}"
    resp_data = response.json()
    submission_id = resp_data["submission_id"]
    
    # Retrieve the exact workflow state from the shared local session service
    sess = await session_service.get_session(
        app_name="app",
        user_id="user_eval",
        session_id=submission_id
    )
    from expense_agent.db import get_submission
    sub = get_submission("org_eval", submission_id)
    sub_status = sub.get("status", "UNKNOWN") if sub else "UNKNOWN"
    
    session_state = sess.state if sess else {}
    
    is_security_event = bool(session_state.get("is_security_event", False))
    status = sub_status
    ...
    if is_security_event:
        ...
        novelty_score = 1
    elif session_state.get("ceiling_override_needed", False):
        status = "CEILING_ESCALATED"
        novelty_score = None
    else:
        report_text = session_state.get("innovation_analysis")
        novelty_score = parse_novelty_score_from_report(report_text)
        if novelty_score is None and status == "PAUSED_FOR_REVIEW":
...
async def evaluate_all_cases() -> Dict[str, Any]:
    ...
    # Setup database and seed evaluation user
    initialize_db()
    ...
    with TestClient(app) as client:
        ...
        try:
            for idx, case in enumerate(eval_cases):
                ...
                if idx > 0:
                    await asyncio.sleep(12)  # Rate limit: safe sleep between cases to allow token bucket recovery
                actual = await run_single_eval_case(case, client, token, session_service)
